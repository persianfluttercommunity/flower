...
typedef FlowerControllerBuilder<T extends LifeCycleMixin> = Widget Function(
    T controller);
...
class FlowerBuilder<T extends Controller> extends StatelessWidget {
  final FlowerControllerBuilder<T> builder;
  final bool global;
  final Object? id;
  final String? tag;
  ...
  Widget build(BuildContext context) {
    ...
    // return widFlower.builder(controller!);
  }
}
...
abstract class Bind<T> extends StatelessWidget {
  ...
  static void reloadAll({bool force = false}) => Flower.reloadAll(force: force);
  ...
  static void replace<P>(P child, {String? tag}) {
    ...
    Flower.register(child, tag: tag, permanent: permanent);
  }
  ...
  static void lazyReplace<P>(InstanceBuilderCallback<P> builder,
    ...
    final permanent = (info.isPermanent ?? false);
    delete<P>(tag: tag, force: permanent);
    Flower.lazyRegister(builder, tag: tag, fenix: fenix ?? permanent);
  }
...
class BindElement<T> extends InheritedElement {
  ...
  void initState() {
    ...
    var isRegistered = Flower.isRegistered<T>(tag: widget.tag);

    if (widget.global) {
      if (isRegistered) {
        if (Flower.isPrepared<T>(tag: widget.tag)) {
          _isCreator = true;
        } else {
          _isCreator = false;
        }

        _controllerBuilder = () => Flower.find<T>(tag: widget.tag);
      } else {
        _controllerBuilder =
            () => (widget.create?.call(this) ?? widget.init?.call());
        _isCreator = true;
        if (widget.lazy) {
          Flower.lazyRegister<T>(_controllerBuilder!, tag: widget.tag);
        } else {
          Flower.register<T>(_controllerBuilder!(), tag: widget.tag);
        }
      }
    } else {
      _controllerBuilder =
  ...
  void dispose() {
    ...
    if (_isCreator! || widget.assignId) {
      if (widget.autoRemove && Flower.isRegistered<T>(tag: widget.tag)) {
        Flower.delete<T>(tag: widget.tag);
      }
    }
...
/// methods (like Flower.to()) have a `binding` property that takes an
/// instance of Bindings to manage the
/// dependencies() (via Flower.put()) for the Route you are opening.
