...
// import 'package:Flower/navigation/router_report.dart';
// import 'package:Flower/state_manager/controller/lifecycle.dart';
// import 'package:Flower/core/smart_management.dart';
...
extension Inst on FlowerInterface {
  T call<T>() => find<T>();
  ...
  void lazyRegister<S>(
    ...
    _insert(
      ...
      permanent: permanent,
      builder: builder,
      fenix: fenix ?? Flower.smartManagement == SmartManagement.keepFactory,
    );
  }
  ...
  S? _initDependencies<S>({String? name}) {
    ...
    if (!isInit) {
      ...
      if (_singl[key]!.isSingleton!) {
        ...
        if (Flower.smartManagement != SmartManagement.onlyBuilder) {
  ...
  S _startController<S>({String? tag}) {
    ...
    if (i is LifeCycleMixin) {
      i.onStart();
      if (tag == null) {
        Flower.log('Instance "$S" has been initialized');
      } else {
        Flower.log('Instance "$S" with tag "$tag" has been initialized');
      }
      if (!_singl[key]!.isSingleton!) {
  ...
  void reloadAll({bool force = false}) {
    _singl.forEach((key, value) {
      if (value.permanent && !force) {
        Flower.log('Instance "$key" is permanent. Skipping reload');
      } else {
        value.dependency = null;
        value.isInit = false;
        Flower.log('Instance "$key" was reloaded.');
      }
    });
  }
  ...
  void reload<S>({
    ...
    builder.dependency = null;
    builder.isInit = false;
    Flower.log('Instance "$newKey" was restarted.');
  }
...
class _InstanceBuilderFactory<S> {
  ...
  void _showInitLog() {
    if (tag == null) {
      Flower.log('Instance "$S" has been created');
    } else {
      Flower.log('Instance "$S" has been created with tag "$tag"');
    }
  }
