...
String? _extractRouteName(Route? route) {
  ...
  if (route is FlowerPageRoute) {
    return route.routeName;
  }

  if (route is FlowerDialogRoute) {
    return 'DIALOG ${route.hashCode}';
  }

  if (route is FlowerModalBottomSheetRoute) {
...
class FlowerObserver extends NavigatorObserver {
  ...
  void didPop(Route route, Route? previousRoute) {
    ...
    //   // Flower.log("CLOSE SNACKBAR ${currentRoute.name}");
    //   Flower.log("CLOSE SNACKBAR");
    // } else

    if (currentRoute.isBottomSheet || currentRoute.isDialog) {
      Flower.log("CLOSE ${currentRoute.name}");
    } else if (currentRoute.isGetPageRoute) {
      Flower.log("CLOSE TO ROUTE ${currentRoute.name}");
    }
  ...
  void didPush(Route route, Route? previousRoute) {
    ...
    //   // Flower.log("OPEN SNACKBAR ${newRoute.name}");
    //   Flower.log("OPEN SNACKBAR");
    // } else

    if (newRoute.isBottomSheet || newRoute.isDialog) {
      Flower.log("OPEN ${newRoute.name}");
    } else if (newRoute.isGetPageRoute) {
      Flower.log("GOING TO ROUTE ${newRoute.name}");
    }
  ...
  void didRemove(Route route, Route? previousRoute) {
    ...
    Flower.log("REMOVING ROUTE $routeName");
  ...
  void didReplace({Route? newRoute, Route? oldRoute}) {
    ...
    Flower.log("REPLACE ROUTE $oldName");
    Flower.log("NEW ROUTE $newName");
    ...
    if (oldRoute is FlowerPageRoute) {
...
class _RouteData {
  ...
  factory _RouteData.ofRoute(Route? route) {
    return _RouteData(
      ...
      isGetPageRoute: route is FlowerPageRoute,
      isDialog: route is FlowerDialogRoute,
      isBottomSheet: route is FlowerModalBottomSheetRoute,
    );
  }
}
