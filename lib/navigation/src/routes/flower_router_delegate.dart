...
class FlowerDelegate extends RouterDelegate<RouteDecoder>
    with
        ChangeNotifier,
  ...
  final FlowerPage notFoundRoute;
  ...
  List<FlowerPage> get registeredRoutes => _routeTree.routes;

  void addPages(List<FlowerPage> flowerPages) {
    _routeTree.addRoutes(flowerPages);
  }
  ...
  void addPage(FlowerPage getPage) {
    _routeTree.addRoute(getPage);
  }

  void removePage(FlowerPage getPage) {
  ...
  Iterable<FlowerPage> getVisualPages(RouteDecoder? currentHistory) {
  ...
  Future<T?> to<T>(
    ...
    final getPage = FlowerPage<T>(
      name: routeName,
      opaque: opaque ?? true,
      page: page,
      gestureWidth: gestureWidth,
      showCupertinoParallax: showCupertinoParallax,
      popGesture: popGesture ?? Flower.defaultPopGesture,
      transition: transition ?? Flower.defaultTransition,
      curve: curve ?? Flower.defaultTransitionCurve,
      fullscreenDialog: fullscreenDialog,
      bindings: bindings,
      transitionDuration: duration ?? Flower.defaultTransitionDuration,
      preventDuplicateHandlingMode: preventDuplicateHandlingMode,
    );
  ...
  Future<T?> _replace<T>(PageSettings arguments, FlowerPage<T> page) async {
